...
from app.models import Customer, DebtPayment, Order
from app import db
...
def customer_detail(customer_id):
    ...
    payments = DebtPayment.query.filter_by(customer_id=customer_id).order_by(DebtPayment.timestamp.desc()).all()
...
def make_payment(customer_id):
    ...
    payment = DebtPayment(customer_id=customer_id, user_id=current_user.id, amount=amount)
